(function (global, factory) {
  ...
  describe("Tokenizer", function () {
    ...
    var withString = function withString() {
      return _fableCore.String.join("", Array.from(_fableCore.Seq.initialize(100, function (_arg1) {
        return randomString("\"'\\\n abcDEF1234,[].+-/<>=@#$%");
      })));
    };

    var withTokens = function withTokens() {
    ...
    var tokensEqual = function tokensEqual(t1, t2) {
      var equal = _fableCore.Seq.toList(_fableCore.Seq.delay(function (unitVar) {
        return _fableCore.Seq.map(function (t) {
          return t.Token;
        }, t1);
      })).Equals(_fableCore.Seq.toList(_fableCore.Seq.delay(function (unitVar) {
        return _fableCore.Seq.map(function (t) {
          return t.Token;
        }, t2);
      })));
      ...
      if (!equal) {
        _fableCore.String.fsFormat("      *** Assertion failed: Tokens do not match ***")(function (x) {
          console.log(x);
        });

        _fableCore.String.fsFormat("      *** Tokens:")(function (x) {
          console.log(x);
        });

        var n = _fableCore.Seq.count(_fableCore.Seq.takeWhile(function (tupledArg) {
          return tupledArg[0].Equals(tupledArg[1]);
        }, _fableCore.Seq.zip(t1, t2)));

        _fableCore.Seq.iterate(function (tupledArg) {
          _fableCore.String.fsFormat("      *** - '%s' %s '%s' ")(function (x) {
            console.log(x);
          })(formatToken(tupledArg[0].Token))(tupledArg[0].Equals(tupledArg[1]) ? "=" : "<>")(formatToken(tupledArg[1].Token));
        }, _fableCore.Seq.truncate(5, _fableCore.Seq.skip(0 > n - 2 ? 0 : n - 2, _fableCore.Seq.zip(t1, t2))));
      }
    ...
    var equal = function equal(a, b) {
      if (!_fableCore.Util.equals(a, b)) {
        _fableCore.String.fsFormat("      *** Assertion failed: Values do not match ***")(function (x) {
          console.log(x);
        });

        _fableCore.String.fsFormat("      *** Expected: %A")(function (x) {
          console.log(x);
        })(a);

        _fableCore.String.fsFormat("      *** Actual: %A")(function (x) {
          console.log(x);
        })(b);
      }

      assert.equal(true, _fableCore.Util.equals(a, b));
    };

    it("Format and tokenize returns original tokens", function () {
      check(function (arg00_) {
        return withTokens(arg00_);
      }, function (sourceTokens) {
        var patternInput = (0, _tokenizer.tokenize)(formatTokens(sourceTokens));
        tokensEqual(patternInput[0], sourceTokens);
      });
    });
    it("Format and tokenize does not produce errors", function () {
      check(function (arg00_) {
        return withTokens(arg00_);
      }, function (sourceTokens) {
        var patternInput = (0, _tokenizer.tokenize)(formatTokens(sourceTokens));
        equal([], patternInput[1]);
      });
    });
    it("Concatenate using ranges recreates original input for valid tokens", function () {
      check(function (arg00_) {
        return withTokens(arg00_);
      }, function (sourceTokens) {
        var input = formatTokens(sourceTokens);
        var patternInput = (0, _tokenizer.tokenize)(input);
        var output = formatTokensUsingRange(input, patternInput[0]);
        equal(input, output);
      });
    });
    it("Concatenate using ranges recreates original input for random string", function () {
      check(function (arg00_) {
        return withString(arg00_);
      }, function (input) {
        var patternInput = (0, _tokenizer.tokenize)(input);
        var output = formatTokensUsingRange(input, patternInput[0]);
        equal(input, output);
